...
- `map.html` は統計表示と業種フィルターだけの最小構成のプレビュー地図。業種の分類は
  ...
  施設の点は `POINT_MIN_ZOOM`（現在 12）以上でしか表示しない**暫定の制限**がある。
  低ズームのタイルは範囲内の施設を間引かずに持つため1枚が極端に重く（z6 の関東〜近畿は
  36.8MB / 724,758点）、z6 で開くと初期表示だけで約57MB 読んで描画に至らないため。
  タイル側の間引きが入ったらこの下限は下げられる
